...
<br>
    ...
    <form class="custom_form" action="{{ route('events.update', $event->id) }}" method="post" enctype="multipart/form-data">
        ...
        <input id="title" type="text" name="start" class="form-control datetime-picker"  value="{{ $event->start ? date('m/d/Y h:i a', strtotime($event->start)) : ''}}">
        <br>

        <label><b>End Date and Time</b></label>
        <input id="title" type="text" name="end" class="form-control datetime-picker"  value="{{ $event->end ? date('m/d/Y h:i a', strtotime($event->end)) : ''}}">
        <br>
        ...
        <br>
        <input type="submit" class="btn btn_custom_style btn-primary" value="UPDATE PLAB EVENT" style="float:right">
    </form>
