...
<div class="panel panel-white">
    ...
    @if(auth()->user()->can('Create Blog'))
        <div class="panel-heading clearfix btn-left">
            <a href="{{ route('notes-bank.create') }}" class="btn btn-sencodary">Add Post</a>
        </div>
    @endif
                        ...
                        <tr>
                            ...
                            <td>
                                @if(auth()->user()->can('Edit Blog'))
                                    <a href="{{ route('notes-bank.edit', $item->id) }}" style="background-color: #0A68D4; color: #fff; border: none;" class="btn btn-sm"><i class="fa fa-edit edit"></i></a>
                                @endif

                                @if(auth()->user()->can('Delete Blog'))
                                    <form method="post" class="delete-form" action="{{ route('notes-bank.destroy', $item->id) }}">
                                        @csrf
                                        @method('DELETE')
